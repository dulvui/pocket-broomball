...
func set_up_teams(teams):
	...
	for team in teams:
		if team["id"] != 0:
			...
			$MarginContainer/MarginContainer/GridContainer.add_child(position_label)
			position += 1
			
			var name_label = Label.new()
			name_label.text = team["name"]
			$MarginContainer/MarginContainer/GridContainer.add_child(name_label)
			
			var points_label = Label.new()
			points_label.text = str(team["points"])
			$MarginContainer/MarginContainer/GridContainer.add_child(points_label)
			
			var wins_label = Label.new()
			wins_label.text = str(team["wins"])
			$MarginContainer/MarginContainer/GridContainer.add_child(wins_label)
			
			var lose_label = Label.new()
			lose_label.text = str(team["lost"])
			$MarginContainer/MarginContainer/GridContainer.add_child(lose_label)
			
			var gd_label = Label.new()
			gd_label.text = str(team["goal_difference"])
			$MarginContainer/MarginContainer/GridContainer.add_child(gd_label)
